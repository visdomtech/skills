...
Example input CSV:
    documentIds,errorMessage?(optional),status,workspaceId
    ...,...,...,...
...
async def main():
    parser = build_argparser(TOOL_NAME, None, "Update the processing status and optional error message of a document.", "Example input CSV:\n    documentIds,errorMessage?(optional),status,workspaceId\n    ...,...,...,...\n\nExample output CSV:\n    result_field1,result_field2,...\n    value1,value2,...\n")
    ...
    async with get_mcp_session(args.config) as session:
        for i, row in enumerate(rows):
            ...
            params = {
            "documentIds": json.loads(row["documentIds"]),
            "errorMessage": row["errorMessage"] if row.get("errorMessage") else None,
            "status": row["status"],
