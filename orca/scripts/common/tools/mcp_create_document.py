...
Example input CSV:
    documentType,fileReferenceId?(optional),fileSizeBytes?(optional),filename,gsUri,repositoryId,uploadedBy,workspaceId
    ...,...,...,...,...,...,...,...
...
async def main():
    parser = build_argparser(TOOL_NAME, None, "Create a new document record in a repository and return its document_id.", "Example input CSV:\n    documentType,fileReferenceId?(optional),fileSizeBytes?(optional),filename,gsUri,repositoryId,uploadedBy,workspaceId\n    ...,...,...,...,...,...,...,...\n\nExample output CSV:\n    result_field1,result_field2,...\n    value1,value2,...\n")
    ...
    async with get_mcp_session(args.config) as session:
        for i, row in enumerate(rows):
            ...
            "fileReferenceId": int(row["fileReferenceId"]) if row.get("fileReferenceId") else None,
            "fileSizeBytes": int(row["fileSizeBytes"]) if row.get("fileSizeBytes") else None,
            "filename": row["filename"],
            "gsUri": row["gsUri"],
            "repositoryId": int(row["repositoryId"]),
