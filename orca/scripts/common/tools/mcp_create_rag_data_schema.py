...
Example input CSV:
    corpusName,dataType,granularity?(optional),key
    ...,...,...,...
...
TOOL_NAME = "create_rag_data_schema"



async def main():
    parser = build_argparser(TOOL_NAME, None, "Create a typed data schema key on a RAG corpus. Must exist before creating metadata with that key.", "Example input CSV:\n    corpusName,dataType,granularity?(optional),key\n    ...,...,...,...\n\nExample output CSV:\n    result_field1,result_field2,...\n    value1,value2,...\n")
    ...
    async with get_mcp_session(args.config) as session:
        for i, row in enumerate(rows):
            ...
            "granularity": row["granularity"] if row.get("granularity") else None,
            "key": row["key"],
            }
